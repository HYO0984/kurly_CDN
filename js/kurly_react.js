...
const WrapComponent = () => {
  ...
  // 모달 상태관리
  const [modal, setModal] = React.useState({title: '', isShow: false});
  ...
  //로그인 상태 변경 함수
  const loginState=()=>{
  setLogin(true);       //로그인 show
  setMember(false);     //회원가입 hide
  }
  //회원가입 상태 변경 함수
  const memberState=()=>{
  setMember(true);   //로그인 hide
  setLogin(false);   //회원가입 show
  }
  ...
  return (
    <div id="wrap">
        <HeaderComponent loginState={loginState} memberState={memberState}/>
        {
          login && (
            <LoginComponent memberState={memberState}/>
          )
        }
        {
          member &&(
      ...
      const HeaderComponent = ({loginState,memberState}) => {
        ...
        //로그인 상태관리
        const [isLogin ,setIsLogin]   = React.useState(false); //로그인 성공하면 true
        const [userName,setUserName]  = React.useState('');     // 로그인 성공하면 이름 출력

        // 1-2. 팝업창 상태관리(state) 멤버변수
        const [popup,setPopup] = React.useState(false); //멤버변수 => 기본값 false - 안보임

        //로그아웃
        //1. 로그인('userId')쿠키삭제 날짜 -1
        //2. 로그인('userId') 로컬스토레이지 데이터 삭제
        //3. setIsLogin(false);
        //4. setUserName(''); 
        const onClickLogout=(e)=>{
          ...
          newDate.setDate(newDate.getDate()-1); // 하루 삭제 => 쿠키삭제
          document.cookie = `${'userId'}=${value};path=/;expires=${newDate.toUTCString()};`;
          //로컬스토레이지삭제
        ...
        //로그인 이벤트 :  로그인 유지
        const loginEvent=()=>{
            //1. 로컬스토레이지 key : userId
            if(localStorage.length <= 0 ) return;
            const localData = JSON.parse(localStorage.getItem('userId')); 
          
            //2. 쿠키이름 : userId
            let result = getCookie('userId');
            if( result === localData.아이디 ){
                setIsLogin(true);
        ...
        const onClickMember=(e)=>{
          e.preventDefault();
          memberState();  // 상위(부모) 컴포넌트의 상태변경 함수 호출 실행
        }

        //로그인 버튼 클릭 이벤트
        const onClickLogin=(e)=>{
          e.preventDefault();
          loginState(); // 상위(부모) 컴포넌트의 상태변경 함수 호출 실행
        }
        ...
        const getCookie=(name)=>{
          let temp = []; //1
          let obj = []; //2
          let found = ''; //리턴으로 no를 보낸다는 임시변수 //3
          // console.log(name);                  //검색하고자하는 쿠키 이름
          // console.log(document.cookie);       //저장된 모든 쿠키, 값 , 만료일
          // console.log(document.cookie.split(';'));       //저장된 모든 쿠키 배열처리 분리한다
          if(document.cookie==='')return;
          temp = document.cookie.split(';');       //저장된 모든 쿠키 배열처리 분리한다 = 1차
          temp.map((item,idx)=>{
            // console.log(item); // 이름, 값을 분리해서 객체에 저장
            obj[idx] = {
               name:item.split('=')[0].trim(), //이름 name 분리해서 객체에 저장
              value:item.split('=')[1].trim()//값 value 분리해서 객체에 저장
            } 
          });
          //반복처리 쿠키 이름을 비교 판단 
          obj.map((item)=>{
        ...
        //3. 팝업창 닫기 +  쿠키설정 (닫기 버튼 onClick={onClickPopupClose})
        const onClickPopupClose =(e)=>{
          e.preventDefault();
          // 3-1. 팝압창 닫기 
          setPopup(false);
          // 3-2. 쿠키 설정(setCookie 셋 쿠키)
              // => 쿠키 데이터(이름(name),값(value),만료기간(expires))를 기록 저장
          function setCookie(name,value,expires){
            let newDate = new Date();
            newDate.setDate(newDate.getDate()+expires); //하루 1일을 더한다 날짜연산
            document.cookie = `${name}=${value};path=/;expires=${newDate.toUTCString()};`;
          }
          //오늘 하루동안 열리지 않음을 설정 - 하루 1일 설정 setDate()
          //setCookie(이름(name),값(value),만료기간(expires))
          setCookie('popup_20220809_event','no',1)
          // setCookie('popup_20220809_event2','no',1)
          // setCookie('popup_20220809_event3','no',1)
          // setCookie('popup_20220809_event4','no',1)
        }

        // 2-1. 팝업창 띄우기(열기) 함수
        const popupOpen=()=>{
          //쿠키 데이터가 있다면 팝업창 열지 않고,
          //쿠키 데이터가 없다면 팝업창 열림
          // 4-1. getCookie() 함수에서 쿠키데이터 유무 검사 리턴값 돌려준다
          // 4-2. 돌려준 값과 비교 팝업창 띄울것인지 안띄울것인지 결정
          const result = getCookie('popup_20220809_event'); // 이 쿠키이름이 있는지 검사
          if(result !== 'no'){ // 리턴값이 no가 아니면 팝업창 띄우기 no 이면 안 띄어준다.
            setPopup(false); //팝업창 안띄우기
          }
          else{
            setPopup(true); //팝업창 띄우기
          }
        }

        // 2-2. 팝업창 멤버변수를 한번만 실행하고 수정하는 상태관리
        // 훅 (useEffect) 사용   ----  한번만 실행 (오류X)
          // ㄴ> 제어하는 메소드 함수
        React.useEffect(()=>{
        //팝업창 띄우기(열기) 함수 실행 
        popupOpen();
            ...
            <div className="container">
              <div className="wrap">
                ...
                <div className="popup-container">
                  ...
                  {/* <!-- 닫기버튼 --> */}
                  <span>
                    <button onClick={onClickPopupClose} className="popup-close-btn" title="닫기">
                ...
                <ul>
                  <li>
                    {
                     isLogin === false ? <a href="#" onClick={onClickMember}>회원가입</a> : <a href="#">웰컴</a>
                    }
                  </li>
                  <li><span>|</span></li>
                  <li>
                    {
                      isLogin === false ? <a href="#" onClick={onClickLogin}>로그인</a> : <a href="#">{userName}님!</a>
                    }
                  </li>
                  <li><span>|</span></li>
                  <li>
                    {
                      isLogin === false ? <a href="#">고객센터</a> : <a href="#"onClick={onClickLogout}>로그아웃</a>
                    }
                  </li>
                </ul>
                </aside>
      ...
      const LoginComponent=({memberState})=>{
        ...
        //로그인 구현 클릭 이벤트
        //1. 입력폼 아이디, 비밀번호를 폼데이터로 전송
        //2. AXIOS를 이용 서버(login.php)에 전송
        //3. 서버(login.php) 아이디, 비밀번호를 데이터베이스 데이터와 비교
        //4. 찾으면 로그인 메시지를 응답하고 로그인 이후 내용을 나타낸다.
        ...
        const axiosEvent=()=>{
          ...
          .then((res)=>{
            ...
            localStorage.setItem('userId', `{"아이디": "${res.data.아이디}", "이름": "${res.data.이름}"}` );
            location.href ='./'; //./index.html
            // location.href ='http://hoo0049.dothome.co.kr/kurly_/'; //현재창
            // window.open ='http://hoo0049.dothome.co.kr/kurly_/'; //새창
          })
          .catch((err)=>{
            console.log(err);
          })
            ...
            <div className="container">
              ...
              <div className="content">
                ...
                <ul>
                  ...
                  <li>
                    <button onClick={onClickLoginEvent} className="login-btn" type='button'>로그인</button>
                  </li>
                  <li>
                    <button onClick={onClickGaibEvent} className="gaib-btn" type='button'>회원가입</button>
                  </li>
                </ul>
                </form>
              </div>
          ...
          <main id="main">
            ...
            const MemberComponent = ({modal, modalShowFn, 이용약관}) => {
              ...
              const onCompletePost=(data)=>{ //카카오 다움 주소검색 API
                setField({...field, isShowAddr: true, 주소1: data.roadAddress}) //도로명주소
              }
              ...
              const [field, setField] = React.useState(
                    {
                      ...
                      //데이터베이스에서 가져온 데이터 저장할 배열
                      //홈페이지가 로딩시에 가져와서 저장한다.
                      가입회원:[]
                  ...
                  if(field.아이디===''){
                    ...
                    if(field.isClassId===false){ //정규표현식이 오류가 있다면
                      modalShowFn('아이디는 6자 이상의 영문 혹은 영문과 숫자를 조합하여 입력하세요!');
                        ...
                        imsi = field.가입회원; //데이터베이스에서 가져온 데이터
              ...
              const onClickAddr=(e)=>{
                  ...
                  new daum.Postcode({
                      oncomplete: function(data) {
                          //주소검색결과 전달 아규먼트
                          onCompletePost(data);  //전달되면서 주소 입력창이 열린다.                        
                      }
                  }).open();
              ...
              // 0. 유효성 검증
              // 1. 로컬스토레이지에 저장(전송)
              // 2. 닷홈 비동기 전송방식 : AXIOS 전송(보내고&받기) => 서버(PHP, MYSQL)와 정보 송수신(CRUD)하기 위해 사용 
                  ...
                  if(아이디==='' || 비밀번호==='' || 비밀번호확인==='' || 이름==='' ||  이메일==='' ||  휴대폰==='' ||  주소1==='' ||  주소2==='' ||   아이디중복확인Ok===false ||  이메일중복확인Ok===false ||  휴대폰인증번호확인Ok===false ){
                    ...
                    if(cnt<3){
                      ...
                      return;
                        ...
                        //프론트단에 저장
                        //본인컴퓨터의 브라우저의 메모리에 저장된다.(로컬 Local)
                        //로컬스토레이지는 데이터저장시 객체를 저장할 수 없다. 
                        //그래서 문자열로 변환(JSON.stringify()) 저장한다.
                        localStorage.setItem(imsi.아이디,  JSON.stringify(imsi) ); 
            

                        //서버단에 저장
                        //웹호스팅의 닷홈의 서버단(데이터베이스=>(화면단에 데이터를 React.JS => AIOS => [PHP(서버단) => MYSQL(서버단) => 데이터베이스(서버단)] )에 저장된다.
                        //AXIOS 서버에게 데이터 전송 구현
                        //비동기식 전송

                        //2. 폼데이터 객체 생성
                        //   - php 서버단에 전송될 속성(변수)에 데이터를 추가(넣어준다)한다.
                          ...
                          formData.append('addinput', `${추가입력사항선택}: ${추가입력사항}`);
                          formData.append('service', 이용약관동의); 
                          formData.append('gaib', `${new Date().getFullYear()}-${new Date().getMonth()+1}-${new Date().getDate()}`);
                          ...
                          .then((response)=>{ //서버단에서 응답 Response                             
                              //저장완료
                              modalShowFn('AXIOS 성공 마켓컬리 회원가입을 진심으로 감사드립니다.');
                              ...
                              setField({  
                                ...
                                isShowAddr: false,  //주소검색 입력상자 
                                isShowAddr2: false, //주소검색 API => 오직 false
                                주소1:'',
                                주소2:'',   
                          ...
                          .catch((error)=>{ //전송시 에러 발생  캐치 catch
                              console.log('AXIOS 실패: ', error );
                              modalShowFn('AXIOS 실패 오류를 확인하세요!');
                          });
             ...
             // 데이터 베이스 에서 데이터 가져오기 GET => select.php
             // axios 함수 : axios는 data 속성 데이터가 온다. => res.data
              // let cnt = 0;
              const axiosGet=()=>{
                axios({
                  url:'./select.php',
      ...
      const ModalComponent = ({modal, modalCloseFn}) => {
        ...
        const onClickClose=()=>{
          modalCloseFn(); //부모 컴퍼넌트의 닫기 함수 호출 실행
        }
...
//1. 돔컨테이너와 컴포넌트 연결 -> 리액트돔 ReactDOM.render()
ReactDOM.render(
  <WrapComponent />,
